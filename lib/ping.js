...
const { pingJavaWithHandshake } = require('./java-ping')
...
function pingJava (serverRegistration, connectHost, connectPort, handshakeHost, handshakePort, timeout, version, callback) {
  pingJavaWithHandshake(
    ...
    if (isProxyStatusFailure(res)) {
      callback(new Error('Proxy request failed'))
      return
    }

    const payload = {
      players: {
        online: capPlayerCount(serverRegistration.data.ip, parseInt(res.players.online, 10))
      },
      version: parseInt(res.version.protocol, 10),
      versionName: res.version.name
    }
  ...
  switch (serverRegistration.data.type) {
    case 'PC':
      serverRegistration.dnsResolver.resolve((host, port, remainingTimeout) => {
        ...
        const connectTimeout = Math.max(remainingTimeout, MIN_CONNECT_TIMEOUT)
        ...
        const tryPing = () => {
          const handshakeHost = attempts[attemptIndex]
          const attemptTimeout = Math.max(
            connectTimeout - (attemptIndex * 500),
            MIN_CONNECT_TIMEOUT
          )
          ...
          pingJava(
            ...
            version,
            (err, resp) => {
              if (err && attemptIndex < attempts.length - 1 && isRetryablePingError(err)) {
                attemptIndex++
                tryPing()
                return
              }
